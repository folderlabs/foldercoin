...
const FolderCoinToken = artifacts.require("FolderCoinToken");
const FolderCoinCrowdsale = artifacts.require("FolderCoinCrowdsale");
...
contract(
  ...
  ([
    ...
    const SYMBOL = "FOL";
    const DECIMALS = 18;
    const INITIAL_SUPPLY = new BigNumber(1250000000); // 1.25 Billion FOLDER_COIN tokens

    const EXCHANGE_RATE = new BigNumber(1000); // 1 ether = 1000 FOLDER_COIN
    const TOTAL_SALE_CAP = ether(10);
    const VESTING_TOKEN_AMOUNT = new BigNumber("4e22"); // 40,000 FOLDER_COIN
    const BONUS_RATE = new BigNumber(2500); // 25% Bonus rate which is multiplied by 100 for better resolution

    // These values are for testing purposes. The values set in FolderCoinCrowdsale
    ...
    const vestAmount_funder = new BigNumber("2.5e21"); // 2,500 FOLDER_COIN
    const vestAmount_thirdParty = new BigNumber("1e21"); // 1,000 FOLDER_COIN
    const vestAmount_thirdPartyAlt = new BigNumber("5e21"); // 5,000 FOLDER_COIN

    beforeEach(async () => {
      this.token = await FolderCoinToken.new(
        NAME,
        SYMBOL,
        DECIMALS,
        INITIAL_SUPPLY
      );
      this.crowdsale = await FolderCoinCrowdsale.new(
        EXCHANGE_RATE,
        wallet,
        this.token.address,
        TOTAL_SALE_CAP
    ...
    describe("Crowdsale::Lockable", () => {
      ...
      const lockedAmount = new BigNumber("200e18"); // 200 FOLDER_COIN tokens
