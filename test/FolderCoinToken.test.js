...
contract("FolderCoinToken", ([creator, receiver, thirdParty]) => {
  const NAME = "FOLDER_COIN Protocol";
  const SYMBOL = "FOLDER_COIN";
  const DECIMALS = 18;
  const INITIAL_SUPPLY = 1250000000; // 1.25 Billion FOLDER_COIN tokens
  const TOTAL_SUPPLY = new BigNumber(INITIAL_SUPPLY * 10 ** DECIMALS);
  const transferAmount = new BigNumber("100e18"); // 100 FOLDER_COIN tokens
  const approveAmount = new BigNumber("10e18"); // 10 FOLDER_COIN tokens
  ...
  describe("ERC20::Standard Token", () => {
    ...
    it("assigns the initial total supply to the creator", async () => {
      ...
      const logs = decodeLogs(receipt.logs, FolderCoinToken, this.token.address);
