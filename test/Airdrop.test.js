...
const Airdrop = artifacts.require("FolderCoinAirdrop");
...
contract("Airdrop", ([creator, addr1, addr2, addr3, addr4, addr5]) => {
  const airdropFund = new BigNumber("10000e18"); // 10000 FOLDER_COIN tokens
  const withdrawalAmount = new BigNumber("100e18"); // 100 FOLDER_COIN tokens
  ...
  describe("Airdrop::functions", () => {
    it("sets token to be airdropped", async () => {
      let newToken = await FolderCoinToken.new(
        NAME,
        SYMBOL,
        DECIMALS,
        INITIAL_SUPPLY
